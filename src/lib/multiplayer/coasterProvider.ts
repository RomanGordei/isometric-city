...
export interface CoasterMultiplayerProviderOptions {
  ...
  onAction?: (action: CoasterAction) => void;
...
export class CoasterMultiplayerProvider {
  ...
  constructor(options: CoasterMultiplayerProviderOptions) {
    ...
    // Create Supabase Realtime channel (use coaster- prefix to avoid collision with city rooms)
  ...
  async connect(): Promise<void> {
    ...
    if (this.isCreator && this.gameState) {
      ...
      try {
        const success = await createCoasterRoom(
          this.roomCode,
      ...
      const roomData = await loadCoasterRoom(this.roomCode);
      if (!roomData) {
    ...
    this.channel
      ...
      .on('broadcast', { event: 'action' }, ({ payload }) => {
        const action = payload as CoasterAction;
  ...
  dispatchAction(action: CoasterActionInput): void {
    ...
    const fullAction: CoasterAction = {
      ...action,
      timestamp: Date.now(),
  ...
  private saveStateToDatabase(state: GameState): void {
    ...
    updateCoasterRoom(this.roomCode, state).catch((e) => {
      if (e instanceof ParkSizeLimitError) {
