...
// 
// Uses the coaster_rooms table (schema same as game_rooms but for IsoCoaster)
// 
// Required Supabase table schema (run this in Supabase SQL editor):
// 
// CREATE TABLE coaster_rooms (
...
// 
// -- Enable RLS
// ALTER TABLE coaster_rooms ENABLE ROW LEVEL SECURITY;
// 
// -- Allow anyone to read/write (for anonymous multiplayer)
// CREATE POLICY "Allow public access" ON coaster_rooms
//   FOR ALL USING (true) WITH CHECK (true);
// 
// -- Auto-update updated_at
// CREATE TRIGGER coaster_rooms_updated_at
//   BEFORE UPDATE ON coaster_rooms
//   FOR EACH ROW
...
/**
 * Serialize and compress game state for database storage
 * Note: Unlike IsoCity, we do synchronous compression here since coaster states are typically smaller
 */
function serializeAndCompress(gameState: GameState): string {
  const json = JSON.stringify(gameState);
  return compressToEncodedURIComponent(json);
}

/**
 * Create a new coaster room in the database
...
export async function createCoasterRoom(
  ...
  try {
    const compressed = serializeAndCompress(gameState);
    ...
    // Re-throw ParkSizeLimitError so callers can handle it specifically
...
export async function loadCoasterRoom(
  ...
  try {
    ...
    return { gameState, parkName: data.park_name };
  } catch (e) {
