...
function generateAdvisorMessages(stats: Stats, services: ServiceCoverage, grid: Tile[][]): AdvisorMessage[] {
  ...
  if (abandonedBuildings > 0) {
    ...
    if (abandonedResidential > 0) details.push(msg('{count} residential', { count: abandonedResidential }));
    if (abandonedCommercial > 0) details.push(msg('{count} commercial', { count: abandonedCommercial }));
    if (abandonedIndustrial > 0) details.push(msg('{count} industrial', { count: abandonedIndustrial }));

    messages.push({
      name: msg('Urban Planning Advisor'),
      icon: 'planning',
      messages: [
        msg('{count} abandoned building{plural} in your city ({details}).', {
          count: abandonedBuildings,
          plural: abandonedBuildings > 1 ? 's' : '',
