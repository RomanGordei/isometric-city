...
export function CoasterProvider({ children, startFresh = false }: { children: React.ReactNode; startFresh?: boolean }) {
  ...
  const [state, setState] = useState<CoasterParkState>(() =>
    createInitialCoasterState(DEFAULT_COASTER_GRID_SIZE, gt('Coaster Park'))
  );
  ...
  const hireStaff = useCallback((type: 'handyman' | 'mechanic' | 'security' | 'entertainer') => {
    ...
    setState((prev) => {
      ...
      const nextStaff = {
        id: nextId,
        name: gt('{staffName} {id}', { staffName: m(definition.name), id: nextId }),
        type,
  ...
  const newGame = useCallback((name?: string, size?: number) => {
    const fresh = createInitialCoasterState(size ?? DEFAULT_COASTER_GRID_SIZE, name ?? gt('Coaster Park'));
    skipNextSaveRef.current = true;
    setState(fresh);
  }, [gt]);
