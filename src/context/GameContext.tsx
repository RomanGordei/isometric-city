...
export function GameProvider({ children, startFresh = false }: { children: React.ReactNode; startFresh?: boolean }) {
  ...
  useEffect(() => {
    // Defer state updates to avoid synchronous setState-in-effect lint + cascading renders
    ...
    const saved = !startFresh ? loadGameState() : null;
  ...
  // Keep latestStateRef in sync without mutating refs during render
  useEffect(() => {
