...
export function useCoasterMultiplayerSync() {
  ...
  const applyRemoteAction = useCallback((action: CoasterAction) => {
    ...
    switch (action.type) {
      ...
      case 'addCoasterTrack':
        game.addCoasterTrack(action.x, action.y);
        break;
  ...
  useEffect(() => {
    ...
    multiplayer.setOnRemoteAction((action: CoasterAction) => {
  ...
  const broadcastSettings = useCallback((settings: Partial<GameState['settings']>) => {
    broadcastAction({ type: 'setSettings', settings });
  }, [broadcastAction]);

  // Helper to broadcast coaster build actions
  const broadcastStartCoasterBuild = useCallback((coasterType: string) => {
    broadcastAction({ type: 'startCoasterBuild', coasterType });
  }, [broadcastAction]);

  const broadcastAddCoasterTrack = useCallback((x: number, y: number) => {
    broadcastAction({ type: 'addCoasterTrack', x, y });
  }, [broadcastAction]);
  ...
  const broadcastPlaceTrackLine = useCallback((tiles: Array<{ x: number; y: number }>) => {
