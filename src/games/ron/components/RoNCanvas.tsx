...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  useEffect(() => {
    ...
    // Initialize frame time inside effect (keeps render pure)
    ...
    const render = () => {
      ...
      const gfx = gameState.graphics ?? { mode: 'realistic', quality: 'balanced' };

      // Draw sky background
      if (gfx.mode === 'realistic') {
        ...
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            ...
            if (tile.hasMetalDeposit) {
              if (gfx.mode === 'realistic') {
                drawEnhancedMountain(ctx, screenX, screenY, x, y, enhancedAnimTimeRef.current, currentZoom);
              } else {
                // Fallback: regular grass base
              ...
              if (gfx.mode === 'realistic') {
                drawEnhancedForest(ctx, screenX, screenY, x, y, tile.forestDensity, enhancedAnimTimeRef.current, currentZoom);
              } else {
                // Fallback to existing sprite-based forest rendering
                drawGroundTile(ctx, screenX, screenY, 'none', currentZoom, false);
              ...
              // Regular grass tile
              if (gfx.mode === 'realistic') {
                drawEnhancedGrassTile(ctx, screenX, screenY, x, y, currentZoom, { highlight: isHovered, selected: isSelected });
              } else {
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            if (gfx.mode === 'realistic') {
              drawEnhancedBeach(ctx, screenX, screenY, x, y, enhancedAnimTimeRef.current, currentZoom, adjacentLand);
            } else {
      ...
      gameState.units.forEach(unit => {
        ...
        const baseColor = PLAYER_COLORS[playerIndex] || '#ffffff';
        const color = gfx.mode === 'realistic' ? mutedTeamColor(baseColor) : baseColor;
