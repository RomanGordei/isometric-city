...
import { createRoNTerrainRenderer } from '../lib/terrain/ronTerrainRenderer';
...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  const terrainRendererRef = useRef<ReturnType<typeof createRoNTerrainRenderer> | null>(null);
  ...
  const [cursorMode, setCursorMode] = useState<'default' | 'grabbing' | 'crosshair'>('default');
  ...
  // Initialize timing refs after mount (avoid impure calls during render)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Lazily initialize terrain renderer (keeps caches across frames)
      if (!terrainRendererRef.current) {
        terrainRendererRef.current = createRoNTerrainRenderer();
      }
      const terrainRenderer = terrainRendererRef.current;
      const showOil = AGE_ORDER.indexOf(playerAge) >= AGE_ORDER.indexOf('industrial');

      // Treat dock footprints as water for terrain adjacency and shore effects.
      const isWaterTile = (gx: number, gy: number) => {
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          // Draw terrain via RoN-specific renderer (realistic materials + animation).
          // Use a water override so dock footprints behave like water for shoreline rendering.
          const effectiveTile = isPartOfDock ? ({ ...tile, terrain: 'water' as const }) : tile;
          ...
          // Fishing spot indicator (keep, but toned down to match new water).
          if ((effectiveTile.terrain === 'water') && tile.hasFishingSpot) {
            const cx = screenX + TILE_WIDTH / 2;
            const cy = screenY + TILE_HEIGHT / 2;

            ctx.save();
            const phase = ((x + y) * 0.27 + timeSeconds) % (Math.PI * 2);
            const rippleSize = 2.8 + Math.sin(phase) * 1.6;
            ctx.globalAlpha = 0.40;
            ctx.strokeStyle = 'rgba(245, 248, 252, 0.85)';
            ctx.lineWidth = 0.8;

            ctx.beginPath();
            ctx.ellipse(cx, cy, rippleSize * 2.1, rippleSize, 0, 0, Math.PI * 2);
            ctx.stroke();
            ctx.beginPath();
            ctx.ellipse(cx, cy, rippleSize * 3.6, rippleSize * 1.7, 0, 0, Math.PI * 2);
            ctx.stroke();

            // Small fish silhouette (subtle).
            ctx.globalAlpha = 0.18;
            ctx.fillStyle = 'rgba(120, 160, 175, 1)';
            const fishX = cx + Math.sin(phase * 2) * 3.2;
            const fishY = cy + Math.cos(phase) * 1.6;
            ctx.beginPath();
            ctx.ellipse(fishX, fishY, 3.6, 1.6, Math.sin(phase) * 0.25, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
          }

          // Optional: forest tree sprites (kept for readability, but color-graded to reduce “cartoon” look).
          if (!isPartOfDock && tile.forestDensity > 0 && currentZoom >= 0.75) {
            ...
            if (isoCitySprite) {
              ...
              const treeSx = 0 * treeTileWidth;
              const treeSy = 3 * treeTileHeight + cropTop;
              const treeSrcHeight = treeTileHeight - cropTop - cropBottom;
              const treeAspect = treeSrcHeight / treeTileWidth;

              const density = Math.min(1, tile.forestDensity / 100);
              const numTrees = 3 + Math.floor(density * 4); // fewer, less “pile of sprites”
              const positions = [
                { dx: 0.5, dy: 0.38 },
                { dx: 0.33, dy: 0.48 },
                { dx: 0.67, dy: 0.46 },
              ...
              ctx.save();
              ctx.filter = 'saturate(0.78) contrast(1.06) brightness(0.95)';
              ctx.globalAlpha = 0.92;
              for (let t = 0; t < numTrees; t++) {
                const pos = positions[t] ?? positions[positions.length - 1];
                const seed = (x * 31 + y * 17 + t * 11) % 100;
                const offsetX = (seed % 10 - 5) * 0.02 * TILE_WIDTH;
                const offsetY = (Math.floor(seed / 10) - 5) * 0.015 * TILE_HEIGHT;
                const treeScale = 0.42 + (seed / 100) * 0.12;
                const treeDestWidth = TILE_WIDTH * treeScale;
                const treeDestHeight = treeDestWidth * treeAspect;

                const drawX = screenX + TILE_WIDTH * pos.dx - treeDestWidth / 2 + offsetX;
                const drawY = screenY + TILE_HEIGHT * pos.dy - treeDestHeight + TILE_HEIGHT * 0.18 + offsetY;

                // Shadow on ground (helps integrate with terrain).
                ctx.save();
                ctx.filter = 'none';
                ctx.globalAlpha = 0.16;
                ctx.fillStyle = '#000';
                ctx.beginPath();
                ctx.ellipse(screenX + TILE_WIDTH * pos.dx + offsetX, screenY + TILE_HEIGHT * (pos.dy + 0.18) + offsetY, 6.5 * treeScale, 3.0 * treeScale, 0, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
          ...
          // Ownership tint overlay (skip for roads; keep subtle).
