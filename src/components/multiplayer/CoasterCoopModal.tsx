...
// Create initial game state for a new park
function createInitialGameState(parkName: string): GameState {
  const gridSize = 60;
  const grid = [];
  for (let y = 0; y < gridSize; y++) {
    ...
    for (let x = 0; x < gridSize; x++) {
      row.push({
        ...
        building: { type: 'empty' as const, level: 0, variant: 0, excitement: 0, intensity: 0, nausea: 0, capacity: 0, cycleTime: 0, price: 0, operating: false, broken: false, age: 0, constructionProgress: 100 },
        path: false,
        queue: false,
        queueRideId: null,
...
export function CoasterCoopModal({
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      const stateToShare = currentGameState 
        ...
        : createInitialGameState(parkName);
  ...
  if (autoJoinError) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md bg-emerald-900 border-emerald-700 text-white">
          <DialogHeader>
  ...
  if (autoJoinAttempted && (isLoading || waitingForState)) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md bg-emerald-900 border-emerald-700 text-white" aria-describedby={undefined}>
          <VisuallyHidden.Root>
          ...
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <T><p className="text-emerald-200">Joining park...</p></T>
            <T><p className="text-emerald-400 text-sm mt-1">Waiting for game state</p></T>
          </div>
        </DialogContent>
      </Dialog>
    );
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md bg-emerald-900 border-emerald-700 text-white">
          ...
          {!roomCode ? (
            <div className="flex flex-col gap-4 mt-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="bg-emerald-800 border-emerald-600 text-white placeholder:text-emerald-500"
                />
              </div>
            ...
            <div className="flex flex-col gap-4 mt-4">
              <div className="bg-emerald-800 rounded-lg p-6 text-center">
        ...
        <div className="flex flex-col gap-4 mt-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              maxLength={5}
              className="bg-emerald-800 border-emerald-600 text-white text-center text-2xl font-mono tracking-widest placeholder:text-emerald-500"
            />
          </div>
          ...
          {waitingForState && (
            <div className="bg-emerald-800/50 rounded-lg p-4 text-center">
              ...
              <T><p className="text-emerald-200 text-sm">Connecting...</p></T>
              <T><p className="text-emerald-400 text-xs mt-1">Waiting for game state</p></T>
            </div>
          )}
