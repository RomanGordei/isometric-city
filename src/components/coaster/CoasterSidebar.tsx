...
export default function CoasterSidebar() {
  ...
  const toolGroups = useMemo<ToolGroup[]>(() => [
    { label: gt('Tools'), tools: ['select', 'path', 'queue_path', 'terrain_raise', 'terrain_lower', 'terrain_smooth', 'coaster_track', 'bulldoze', 'water'] },
    ...
    { label: gt('Shops'), tools: ['shop_food', 'shop_drink', 'shop_ice_cream', 'shop_souvenir', 'shop_info', 'shop_atm', 'shop_first_aid', 'shop_toilet', 'shop_staff_room'] },
  ], [gt]);
          ...
          <div key={group.label} className="mb-4">
            ...
            <div className="px-2 flex flex-col gap-0.5">
              {group.tools.map((tool) => {
                ...
                const costLabel = !isUnlocked ? gt('Research') : info.cost > 0 ? gt('${dollars, number, ::currency/USD}', { dollars: info.cost }) : '';
                return (
                  <Button
                    key={tool}
