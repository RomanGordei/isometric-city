    ...
    <div className="w-56 bg-slate-900/90 backdrop-blur-sm border-r border-white/10 flex flex-col h-screen fixed left-0 top-0 z-40">
      ...
      <div className="px-4 py-4 border-b border-white/10">
        <div className="flex items-center justify-between">
          <T><span className="text-white font-bold tracking-tight">COASTER TYCOON</span></T>
          {onExit && (
            <Button
              variant="ghost"
              size="icon"
                ...
                {availableRides.map(rideType => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs opacity-60">{gt('${cost}k', { cost: (def.buildCost / 1000).toFixed(0) })}</span>
                    </Button>
                  );
                })}
              </HoverSubmenu>
