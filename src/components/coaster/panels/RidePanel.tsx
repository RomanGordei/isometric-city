...
export default function RidePanel({ ride, onClose, onToggleStatus, onPriceChange }: RidePanelProps) {
  ...
  // Sync local price when ride.price changes externally
  if (lastRidePriceRef.current !== ride.price) {
    lastRidePriceRef.current = ride.price;
    setLocalPrice(ride.price);
  }

  const price = localPrice;
  const setPrice = setLocalPrice;
        ...
        <div className="flex items-start justify-between p-4 border-b border-border/60">
          <T>
            <div>
              ...
              <div className="text-lg font-semibold"><Var>{m(ride.name)}</Var></div>
            </div>
          </T>
