...
const STATUS_STYLES: Record<string, { label: string; className: string }> = {
  ...
  storm_closed: { label: msg('Storm Closed'), className: 'text-sky-300' },
};
              ...
              {sortedRides.map((ride) => {
                ...
                return (
                  <div key={ride.id} className="flex items-start justify-between gap-3">
                    <div>
                      <T><div className="font-medium"><Var>{m(ride.name)}</Var></div></T>
                      <T>
                      ...
                      <T>
                        <div className="text-xs text-muted-foreground">
                          <Var>{estimatedWait > 0 ? gt('{estimatedWait} min wait', { estimatedWait }) : gt('No wait')}</Var> · $<Var>{ride.price}</Var> ticket ·{' '}
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        {ride.weatherClosed ? gt('Storm') : ride.status === 'open' ? gt('Close') : gt('Open')}
                      </Button>
                    </div>
                  </div>
                );
