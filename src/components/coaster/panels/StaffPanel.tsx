...
export default function StaffPanel({
  ...
  const staffTypeLabels: Record<string, string> = {
    handyman: gt('handyman'),
    mechanic: gt('mechanic'),
    security: gt('security'),
    entertainer: gt('entertainer'),
  };
          ...
          <T>
            <div className="flex items-center justify-between text-xs uppercase tracking-[0.18em] text-muted-foreground">
              ...
              <span className="text-[10px] text-muted-foreground">
                Staff Rooms <Var>{staffRoomCount}</Var>
              </span>
            </div>
          </T>
          {assignmentTarget && (
                  ...
                  <div key={member.id} className="flex items-start justify-between gap-2 text-sm">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground capitalize">
                        {staffTypeLabels[member.type]} · {member.patrolArea ? gt('Patrol area') : gt('Park-wide')}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-1 text-xs text-muted-foreground">
                      <div>{gt('${wage}/wk', { wage: member.wage })}</div>
                      <div>{gt('Fatigue {fatigue}%', { fatigue: Math.round((member.fatigue / 255) * 100) })}</div>
                      <div className="flex items-center gap-1">
                        <Button
                          size="sm"
