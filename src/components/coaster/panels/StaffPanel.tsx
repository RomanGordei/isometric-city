...
export default function StaffPanel({
  ...
  const getStaffTypeName = (type: string) => {
    const definition = STAFF_DEFINITIONS.find((def) => def.type === type);
    return definition ? m(definition.name) : type;
  };
                ...
                <div key={member.id} className="flex items-start justify-between gap-2 text-sm">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground capitalize">
                      {getStaffTypeName(member.type)} · {member.patrolArea ? gt('Patrol area') : gt('Park-wide')}
                    </div>
                  </div>
