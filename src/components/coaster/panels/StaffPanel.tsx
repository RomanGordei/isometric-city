        ...
        <div className="p-4 space-y-4 text-sm">
          ...
          <ScrollArea className="h-48 rounded-md border border-border/50">
            <div className="p-3 space-y-2">
              ...
              {staff.map((member) => {
                const definition = STAFF_DEFINITIONS.find((def) => def.type === member.type);
                return (
                  <div key={member.id} className="flex items-center justify-between text-sm">
                    <div>
                      <div className="font-medium">{member.name}</div>
                      <div className="text-xs text-muted-foreground">{definition ? m(definition.name) : member.type}</div>
                    </div>
                    <T>
                      <div className="text-xs text-muted-foreground"><Var>{member.wage}</Var>/wk</div>
                    </T>
                  </div>
                );
              })}
