        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-border/50 bg-slate-900/70">
          <div className="flex items-center gap-4">
            ...
            <div className="text-xs text-muted-foreground">
              <T>Year <Var>{state.year}</Var> · Day <Var>{state.day}</Var> · <Var>{state.hour.toString().padStart(2, '0')}</Var>:00</T>
            </div>
          </div>
          <div className="flex items-center gap-4 text-sm">
            <div><T>Guests: <Var>{state.stats.guestsInPark}</Var></T></div>
            <div><T>Rating: <Var>{state.stats.rating}</Var></T></div>
            <div className="capitalize text-muted-foreground">
              <T><Var>{state.weather.type}</Var> · <Var>{state.weather.temperature}</Var>°C</T>
            </div>
